...
std::vector<DetectedObject> ObjectDetector::infer(const cv::Mat &frame) {
  ...
  // Preprocessing untuk SSDLite/YOLOv8 ONNX:
  // scale = 1.0 / 255.0, size = 320x320, swapRB = true
  cv::Mat blob;
  if (isYolo_ || classes_.size() == 3) {
    blob = cv::dnn::blobFromImage(frame, 1.0 / 255.0, cv::Size(320, 320),
                                  cv::Scalar(0, 0, 0), true, false);
  } else {
    // Untuk Caffe MobileNetSSD lama
    blob =
  ...
  if (isYolo_) {
    // Mode YOLOv8: Output shape = [1, 6, 2100] atau [1, 6, 8400]
    cv::Mat output = net_.forward();
    if (!output.empty() && output.dims >= 3) {
      // Sediakan Mat sementara untuk wrapper data asli
      cv::Mat outMatTemp(output.size[1], output.size[2], CV_32F,
                         output.ptr<float>());
      // Lakukan transpose ke Mat terpisah (menghindari in-place corruption pada
      // wrapper)
      cv::Mat outMat;
      cv::transpose(outMatTemp, outMat); // Menjadi [candidates, 6]

      const int numDetections = outMat.rows;

      for (int i = 0; i < numDetections; ++i) {
        // Gunakan outMat.at<float>(i, col) yang 100% aman dari pointer mismatch
        ...
        // Koordinat YOLOv8: [center_x, center_y, width, height] dalam skala 320
    ...
    // Mode ONNX model baru dengan output terpisah "boxes" dan "scores"
    ...
    // Format output ONNX: boxes = [1, 3234, 4], scores = [1, 3234, 3]
    ...
    for (int i = 0; i < numDetections; ++i) {
      ...
      // Cari kelas non-background (1 = car, 2 = motor) dengan score tertinggi
      ...
      // Dekode koordinat kotak (boxesData berskala pixel 320x320)
    ...
    // Mode model Caffe MobileNetSSD lama
    ...
    for (int i = 0; i < detections.size[2]; ++i) {
      ...
      if (classId != 7 && classId != 14) { // Caffe IDs
        continue;
      }

      // Map Caffe ID to classes_
  ...
  for (int idx : keepIndices) {
    ...
    if (bestMatch >= 0 && bestIou >= kIouMatchThreshold) {
      // Jika jarak baru lebih kecil dibanding jarak sebelumnya, berarti
      // mendekat
      object.isApproaching =
          (distanceCm < previousDetections_[bestMatch].distanceCm);

      object.box = smoothBox(previousDetections_[bestMatch].box, object.box,
                             kTemporalAlpha);
      object.distanceCm =
          smoothDistance(previousDetections_[bestMatch].distanceCm,
                         object.distanceCm, kTemporalAlpha);
    } else {
      // Deteksi baru, asumsikan mendekat demi keamanan
