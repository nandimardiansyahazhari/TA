...
void putTextWithBackground(Mat &img, const string &text, Point pos,
                           int fontFace, double fontScale, Scalar textColor,
                           Scalar bgColor, int thickness = 1) {
  int baseline = 0;
  Size textSize = getTextSize(text, fontFace, fontScale, thickness, &baseline);
  baseline += thickness;
  Rect bgRect(pos.x, pos.y - textSize.height, textSize.width,
...
int main() {
  ...
  string model = "/home/ansyah/TA-main/TA_Lite/MobileNetSSD_deploy.caffemodel";
  string config = "/home/ansyah/TA-main/TA_Lite/MobileNetSSD_deploy.prototxt";
  ...
  // Set window sizes for better visibility on laptop
  namedWindow("Pi Collision Warning", WINDOW_NORMAL);
  namedWindow("FPS Graph", WINDOW_NORMAL);
  resizeWindow("Pi Collision Warning", 1280, 720);
  resizeWindow("FPS Graph", 640, 400);
  ...
  while (cap.read(frame)) {
    ...
    if (detectionsAvailable) {
      ...
      for (int i = 0; i < detections.size[2]; i++) {
        ...
        if (confidence > 0.1) {
          ...
          if (classId < classes.size() &&
              (classes[classId] == "car" || classes[classId] == "motorbike")) {
            int left = static_cast<int>(detections.ptr<float>(0)[i * 7 + 3] *
                                        frame.cols);
            int top = static_cast<int>(detections.ptr<float>(0)[i * 7 + 4] *
                                       frame.rows);
            int right = static_cast<int>(detections.ptr<float>(0)[i * 7 + 5] *
                                         frame.cols);
            int bottom = static_cast<int>(detections.ptr<float>(0)[i * 7 + 6] *
                                          frame.rows);

            // Clamp to frame
            left = max(0, left);
            top = max(0, top);
            right = min(frame.cols, right);
            bottom = min(frame.rows, bottom);
            ...
            double W_real =
                (classes[classId] == "car") ? W_real_car : W_real_motor;
            ...
            string label =
                classes[classId] + " " + to_string((int)distance) + "cm";
            putText(frame, label, Point(left, top - 5), FONT_HERSHEY_SIMPLEX,
      ...
      if (closest_distance <= danger_distance && closest_center.x != -1) {
        putTextWithBackground(
            frame, "WARNING! " + to_string((int)closest_distance) + "cm",
            Point(10, 30), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(0, 0, 255),
            Scalar(255, 255, 0), 2);
      }
    }
    ...
    double totalRunTime =
        duration_cast<duration<double>>(now - programStart).count();
    ...
    string timeLabel =
        format("Run Time: %02d:%02d:%02d", hours, minutes, seconds);
    putTextWithBackground(graphImg, timeLabel, Point(10, 30),
                          FONT_HERSHEY_SIMPLEX, 0.6, Scalar(0, 0, 0),
                          Scalar(255, 255, 255));
    ...
    if (!fpsHistory.empty()) {
      ...
      for (size_t i = 0; i < fpsHistory.size(); i++) {
        ...
        int barHeight = static_cast<int>(
            (val / maxFPS) * (graphHeight - 50)); // Leave some margin
        rectangle(graphImg, Point(i * barWidth, graphHeight - barHeight),
                  Point((i + 1) * barWidth - 1, graphHeight), Scalar(0, 255, 0),
                  FILLED);
      }

      putTextWithBackground(graphImg, "Max FPS: " + to_string((int)maxFPS),
                            Point(10, 60), FONT_HERSHEY_SIMPLEX, 0.5,
                            Scalar(0, 0, 255), Scalar(0, 0, 0));
    }
